...
export async function scrapeUrlToProduct(
  ...
  const [store] = await retrieveStores({ domains: [hostname] });
  if (!store) return err({ code: 'INTERNAL', message: 'Store not found after scrape' });

  const [saved] = await retrieveProducts({
    storeIds: [store.id],
...
export async function createLinkFromUrl(
  ...
  const [link] = await createLinks([
    { kind: 'single', merchantId, productIds: [product.value.productId] },
  ]);
...
export async function createPollLinkFromUrls(
  ...
  const [link] = await createLinks([
    { kind: 'poll', merchantId, productIds: products.map((p) => p.productId) },
  ]);
