...
- [x] Broader sweep done. Batched: `enqueueJob` calls in `refresh-catalogs` cron and drop creation (new `enqueueJobs` bulk helper), three N+1 lookups plus the `deliveredAt` write in campaign finalize. Deliberately left alone with real reasons: the job drain loop (each job's outcome is genuinely different), the ESP push loop (no bulk endpoint exists on the hand-rolled Klaviyo/Mailchimp adapters, would have been guessing), the render provider fallback chain (sequential by design), and the poll-link scraping loop (short-circuits on first failure). Commits `17cd1cb`, `525d6c5`, `3ea1bda`.
- Flagged by two independent passes now: `AGENTS.md` claims it's "written and re-added by `next dev`" and points at `node_modules/next/dist/server/lib/generate-agent-files.js` for verification. That file doesn't exist in `node_modules`. The claim about its own provenance appears false, worth a human look since it's checked into the repo and other AI agents may trust its instructions.
