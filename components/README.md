...
`animated?: boolean` on `Timeline` switches it from the pipeline-status stepper (`steps: { label, state }[]`) to a scroll-driven narrative layout for "how it works" sections: a left column with an SVG line whose `pathLength` is tied to `useScroll`/`useTransform` (`stroke-border` base, `stroke-foreground` progressed portion), each item's dot scaling in and content `BlurFade`-ing in on `whileInView`, and a `media?: ReactNode` per step that sticks (`lg:sticky lg:top-24`) and crossfades via `Presence` on desktop, or renders inline per-item below `lg`. `onStepChange?: (index: number) => void` fires as the scroll-derived active step changes. `<Timeline animated steps={[{ label: 'Paste', title: '...', description: '...', media: <X /> }]} onStepChange={track} />`
